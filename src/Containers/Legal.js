...
const LegalContainer = ({route, navigation}) => {
  ...
  useEffect(() => {
    if (legal?.data) {
      ...
      Object.keys(legal.data.agreements).forEach(key => {
        _agreements[key] = {accepted: accepted || agreements[key]?.accepted, label: legal.data.agreements[key]}
      })
  ...
  useEffect(() => {
    if (Object.keys(agreements).length && Object.keys(agreements).map(key => agreements[key]).every(el => el.accepted)) {
      if (!accepted) {
    ...
    } else if (!Object.keys(agreements).map(key => agreements[key]).every(el => el.accepted)) {
      accepted && navigation.reset({index: 0, routes: [{name: 'legal'}]})
      dispatch(Accept.action(false))
    }
  }, [accepted, agreements])

  const agree = useCallback(agreement => {
    setStay(false)
    const _agreements = {
      ...agreements,
    }
        ...
        {Object.keys(agreements).map(key => (
          <TouchableOpacity
            ...
            <View style={[Gutters.smallTMargin, {
              ...Fonts.iconSmall, 
              borderColor: agreements[key].accepted ? Colors.primary : Colors.legal, 
              borderWidth: 1, 
              marginRight: 10
            }]}>
              {agreements[key].accepted ? <Icon name='checkmark-outline' size={Fonts.iconSmall.width*0.8} 
              style={{
                lineHeight: Fonts.iconSmall.height, 
                paddingLeft: 1}}/> : null }
            </View>
            <Text style={[Fonts.textRegular,Layout.fill]}>{agreements[key].label}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </ScrollView>
